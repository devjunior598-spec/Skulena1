...
export async function getPublicSchools(): Promise<School[]> {
  ...
  const [{ data: branches }, { data: levels }, { data: curricula }, { data: facilities }, { data: fees }, { data: verification }, { data: media }] = await Promise.all([
    ...
    supabase.from("school_media").select("school_id, storage_path, is_cover, sort_order").in("school_id", ids).eq("moderation_status", "approved").order("sort_order"),
  ]);
  ...
  return rows.map((row) => {
    ...
    const schoolMedia = (media ?? []).filter((item) => item.school_id === row.id); const cover = schoolMedia.find((item) => item.is_cover) ?? schoolMedia[0];
