...
export async function getOnboardingDraft() {
  ...
  if (!school) return { values: blankOnboarding, completion: 0 };
  const [{ data: branch }, { data: levelRows }, { data: curriculumRows }, { data: facilityRows }, { data: fee }, { data: requirements }, { data: completion }] = await Promise.all([
