...
export function ProfileGallery({ media, variant = "grid" }: { media: ProfileMedia[]; variant?: "hero" | "grid" }) {
  ...
  if (!selected) return <div role="status" className="grid min-h-48 place-items-center rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center text-sm text-slate-600">No photos have been added to this school profile yet.</div>;
    ...
    {variant === "hero" ? (
      <div className={`relative grid h-[290px] gap-2 overflow-hidden rounded-3xl sm:h-[420px] ${media.length > 1 ? "sm:grid-cols-2 lg:grid-cols-4 lg:grid-rows-2" : "grid-cols-1"}`}>
        {media.slice(0, 4).map((item, index) => (
          <button key={item.id} type="button" onClick={() => showPhoto(index)} aria-label={`Open ${item.category.toLowerCase()} photo`} className={`group relative min-h-0 overflow-hidden bg-slate-100 focus-visible:z-10 focus-visible:outline-4 focus-visible:outline-emerald-600 ${index === 0 ? (media.length > 1 ? "sm:row-span-2 lg:col-span-2" : "") : `hidden sm:block ${index === 1 ? "lg:col-span-2" : ""}`}`}>
      ...
      <div>
        ...
        </button>)}</div> : <p className="mt-4 rounded-2xl border border-dashed border-slate-300 p-6 text-sm leading-6 text-slate-600">No photos have been added to this category yet.</p>}
      </div>
    )}
