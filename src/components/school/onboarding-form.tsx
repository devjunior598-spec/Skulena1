...
import { saveOnboardingStep, submitSchool } from "@/app/for-schools/register/actions";
  ...
  return <div className="grid gap-6 lg:grid-cols-[210px_minmax(0,1fr)]"><ProgressSidebar currentStep={step} completion={completion} completedSteps={completedSteps} locked={hasUnuploadedMedia} onNavigate={setStep} />
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-[0_24px_60px_-45px_rgba(14,41,70,.45)] sm:p-8"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-widest text-emerald-700">Step {step} of 10</p><h1 className="mt-2 text-2xl font-extrabold tracking-tight text-[#0e2946]">{steps[step - 1]}</h1></div><p aria-live="polite" className="flex items-center gap-2 text-xs font-bold text-slate-500">{status === "saving" ? <><LoaderCircle className="size-4 animate-spin" />Saving…</> : status === "saved" ? <><Cloud className="size-4 text-emerald-700" />Progress saved</> : "Saved as a draft"}</p></div>
      <div className="mt-7 space-y-5">
        ...
        {step === 2 && <><div className="grid gap-5 sm:grid-cols-2"><label className="block text-sm font-bold">Country<input {...register("country")} className={input} /></label><label className="block text-sm font-bold">State<input {...register("state")} className={input} /></label><label className="block text-sm font-bold">City / LGA<input {...register("city")} className={input} /></label><label className="block text-sm font-bold">Area<input {...register("area")} className={input} /></label></div><label className="block text-sm font-bold">Full address<textarea {...register("addressLine")} className={textarea} rows={3} /></label><details className="rounded-xl bg-slate-50 p-4"><summary className="text-sm font-bold">Map coordinates (optional)</summary><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="text-sm font-bold">Latitude<input {...register("latitude")} className={input} /></label><label className="text-sm font-bold">Longitude<input {...register("longitude")} className={input} /></label></div></details></>}
        {step === 3 && <fieldset><legend className="mb-4 text-sm leading-6 text-slate-600">Select every level currently offered.</legend><CheckGrid name="levels" options={levelOptions} register={register} /></fieldset>}
        ...
        {step === 5 && <fieldset><legend className="mb-4 text-sm leading-6 text-slate-600">Select all curricula used at the school.</legend><CheckGrid name="curricula" options={curriculumOptions} register={register} /></fieldset>}
        ...
        {step === 10 && <div className="space-y-4"><p className="rounded-xl bg-sky-50 p-4 text-sm leading-6 text-sky-950">Review your information before submitting. You can select any step from the progress list to make changes.</p>{[["School", values.name], ["Location", [values.area, values.city, values.state].filter(Boolean).join(", ")], ["Levels", values.levels.join(", ") || "Not added"], ["Curriculum", values.curricula.join(", ") || "Not added"], ["Facilities", `${values.facilities.length} selected`], ["Admissions", values.admissionStatus.replaceAll("_", " ")]].map(([label, value]) => <div key={label} className="border-b border-slate-100 pb-3"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">{label}</p><p className="mt-1 text-sm font-bold text-[#0e2946]">{value}</p></div>)}</div>}
      </div>
