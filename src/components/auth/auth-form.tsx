...
export function AuthForm({ action, mode }: { action: AuthAction; mode: "sign-in" | "sign-up" | "forgot" | "reset" }) {
  ...
  return <form action={formAction} className="space-y-5" noValidate>
    ...
    {mode !== "reset" && <label className="block text-sm font-bold text-slate-700">Email address<input name="email" type="email" autoComplete="email" className={inputClass} required /></label>}
